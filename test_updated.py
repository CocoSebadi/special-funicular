...
def main():
    ...
    if page=='View Current Issues':
        ...
        df=pd.DataFrame(view_all_issues(),columns=["ID",'issue_code','issue_name',' description','issue_status','Principle_risk_type','subrisk_type','entities','bu_rating','agl_rating','assurance_provider','due_date','review_name','financially_implicated','risk_event_type','issue_owner_name','issuer_surname','issuer_owner_email','username'])
    ...
    if page == "Login":
        ...
        if st.session_state.get('username'):
            ...
            Principal_risk_type = st.selectbox("Risk Type", ["Operational & Resilience Risk", "Insurance risk type", "Compliance Risk", "Model Risk", "Conduct Risk"])
            ...
            # Radio button for selecting financial statement type
            financial_statement_type = st.radio("Select Financial Statement Type:", ["Balance Sheet", "Income Statement"])
            # Input field for the amount if fraud element is selected
            fraud_element = st.radio("Is fraud involved in the issue?", ["Yes", "No"])
            if fraud_element == "Yes":
               fraud_amount = st.text_input("Amount (if known)")
            ...
            if st.button("Log Issue"):
                ...
                log_issue(issue_code, issue_name, description, issue_status,Principle_risk_type,subrisk_type,entities, bu_rating,agl_rating,assurance_provider, due_date,financially_implicated,issue_Owner_name,issue_owner_surname, issue_Owner_email,review_name,st.session_state.username)
